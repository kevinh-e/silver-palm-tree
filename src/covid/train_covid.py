from dataLoader import CovidRadiographyDataset
from resnet_covid import ResNet, ResidualBlock
...
DATAPATH = "./data/COVID/COVID-19_Radiography_Dataset"
MODELPATH = "./models/RESNET_COVID"
...
def ResNet20():
    """
    Creates a ResNet-20 Model for COVID dataset
            (n=3)
    Returns:
        (ResNet)
    """
    # 6n + 2 = 6*3 + 2 = 20
    return ResNet(ResidualBlock, [3] * 6)


def ResNet32():
    """
    Creates a ResNet-32 Model for COVID dataset
            (n=5)
    Returns:
        (ResNet)
    """
    # 6n + 2 = 6*5 + 2 = 32
    return ResNet(ResidualBlock, [5] * 6)


def ResNet44():
    """
    Creates a ResNet-44 Model for COVID dataset
            (n=7)
    Returns:
        (ResNet)
    """
    # 6n + 2 = 6*7 + 2 = 44
    return ResNet(ResidualBlock, [7] * 6)


def ResNet56():
    """
    Creates a ResNet-56 Model for COVID dataset
            (n=9)
    Returns:
        (ResNet)
    """
    # 6n + 2 = 6*9 + 2 = 44
    return ResNet(ResidualBlock, [9] * 6)


def ResNet110():
    """
    Creates a ResNet-110 Model for COVID dataset
            (n=18)
    Returns:
        (ResNet)
    """
    # 6n + 2 = 6*18 + 2 = 110
    return ResNet(ResidualBlock, [18] * 6)
...
def load_data(path):
    """Load Data from COVID dataset

    Args:
        path (str): stored data path (PATH)
    """
    covid_dataset = CovidRadiographyDataset(
        root_dir=DATAPATH, apply_horizontal_flip=True
    )
    trainset, testset = random_split(
        covid_dataset, [0.9, 0.1], torch.Generator().manual_seed(42)
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        n_epochs = epochs
        output = f"covid_{model}_{epochs}"

        # Set GPU if is_available
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        writer = SummaryWriter(f"runs/COVID/{args.modelname}|{n_epochs}")
        ...
        # iterations_per_epoch = math.ceil( / batch_size)  # ~391
        ...
        # print("Usage $python src/covid/train_covid.py modelname [epochs] [output]")
        print(f"Error: {e}")
