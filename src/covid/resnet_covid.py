...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=4):
        ...
        # First conv layer [3x3 kernel | input_channels=3 | output_planes=16 | padding = 1 | stride = 2]
        # Downsampled 2x
        self.conv1 = nn.Conv2d(4, 16, kernel_size=3, stride=2, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(16)

        # Create ResidualBlock layers
        # [64x64 -> 16 filters | num_blocks = num_blocks[0] | stride = 2] * downsampled 2x
        self.layer1 = self._make_layer(block, 16, num_blocks[0], stride=2)

        # [32x32 -> 32 filters | num_blocks = num_blocks[1] | stride = 2] * downsampled 2x
        self.layer2 = self._make_layer(block, 32, num_blocks[1], stride=2)

        # [16x16 -> 64 filters | num_blocks = num_blocks[2] | stride = 3] * downsampled 2x
        self.layer3 = self._make_layer(block, 64, num_blocks[2], stride=2)

        # [8x8 -> 128 filters | num_blocks = num_blocks[2] | stride = 2] * downsampled 2x
